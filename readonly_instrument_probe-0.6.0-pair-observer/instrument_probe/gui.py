...
def summarize_report(report: dict[str, object]) -> dict[str, object]:
    ...
    if profile == "6221":
        ...
        # Despite the attribute name, the 2450 manuals define smu.ON as an
        # asserted/engaged safety-interlock signal.  smu.OFF means the signal
        # is not asserted, so high-voltage availability may be restricted.
        interlock = "Engaged / asserted" if interlock_raw in {"1", "smu.ON"} else "Open / not asserted" if interlock_raw in {"0", "smu.OFF"} else interlock_raw or "—"
        if output in {"ON", "smu.ON", "1"}:
            warnings.append("2450 source output is currently ON")
        if interlock_raw in {"0", "smu.OFF"}:
            warnings.append("2450 interlock signal is not asserted")
    else:
...
class InstrumentProbeApp:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(
            controls,
            text="Open Pair Observer",
            command=self._open_pair_observer,
    ...
    def _open_pair_observer(self) -> None:
        from .pair_gui import PairObserverApp

        window = self.tk.Toplevel(self.root)
        PairObserverApp(window)
