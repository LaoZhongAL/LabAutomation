...
这些地址已由 2026-08-19 的 NI MAX 现场截图确认。0.4.1 真实模式只接受这六个地址，并验证所选 profile 与地址预期型号一致。`--identify-lab` 仍会各发送一次 `*IDN?`，用序列号/固件回复建立可审计的当天身份基线；它不会修改地址。
...
项目 0.4.1 的兼容下限是 Python 3.9。Python 3.9 本身不会改变仪器安全性；它只影响代码和 PyVISA 包的可安装版本。Python 位数也不再作为硬性门槛，必须由后续 VISA 枚举实际验证它能否加载现有 NI-VISA。
