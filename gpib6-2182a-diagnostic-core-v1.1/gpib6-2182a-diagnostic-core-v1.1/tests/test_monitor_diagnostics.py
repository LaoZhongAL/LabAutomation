...
class MonitorDiagnosticIntegrationTests(unittest.TestCase):
    ...
    def test_manifest_and_event_log_exist_before_configuration_query(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.object(
                ...
                app._configuration_worker("simulate", "nominal", output_root)
            ...
            self.assertEqual(len(context.query_history), len(monitor.CONFIG_QUERIES))
    ...
    def test_configuration_fault_matrix(self):
        cases = {
            "nominal": (True, "PASS", 22),
            "wrong_identity": (False, "BLOCKED", 1),
            "malformed_identity": (False, "BLOCKED", 1),
            "configuration_missing": (False, "BLOCKED", 22),
            "configuration_drift": (False, "BLOCKED", 22),
            "configuration_timeout": (False, "BLOCKED", 7),
            "configuration_slow": (True, "WARN", 22),
        }
    ...
    def test_csv_open_failure_opens_no_live_session(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            app._stream_worker("simulate", csv_path, context)

            self.assertFalse(csv_path.exists())
            self.assertEqual(context.query_history, [])
            queued_events = []
            while not app.events.empty():
                queued_events.append(app.events.get_nowait()[0])
    ...
    def test_fetch_fault_ends_active_intervention_before_stream_finalization(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            worker = threading.Thread(
                ...
                args=("simulate", csv_path, context, journal, stream_id),
            )
            worker.start()
            ...
            stream_error_payload = next(
                payload for event, payload in queued if event == "stream_error"
            )
    ...
    def test_slow_fetch_transitions_degraded_then_recovers(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            while observed_samples < 3 and time.monotonic() < deadline:
                event, _payload = app.events.get(timeout=1.0)
                if event == "sample":
                    observed_samples += 1
            app.stop_event.set()
    ...
    def test_stream_finalization_failure_is_fault_latched(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            while not app.events.empty():
                queued.append(app.events.get_nowait())
            self.assertIn("stream_error", [event for event, _payload in queued])
            stopped = next(payload for event, payload in queued if event == "stream_stopped")
    ...
    def test_stream_session_open_failure_latches_without_query(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("stream_error", [event for event, _payload in events])
            journal.finalize("test_complete")
