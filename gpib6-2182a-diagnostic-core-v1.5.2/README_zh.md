# GPIB0 多仪器只读诊断与 GPIB6 2182A Live v1.5.2

本目录包含 v1.5.2 动态 instrument inventory、按精确型号复用的只读诊断 profile，以及唯一获批的 GPIB6 Keithley 2182A 实时电压 GUI。v1.5.2 保留 v1.1 的记录语义：电压 CSV 只保存仪器样本，人工干预的类型、位置和开始/结束区间只保存到独立 `interventions.jsonl`。

v1.5.2 保留 v1.5.1 的 15 样本 B0 观察窗和原始证据，只修正一个现场证实的 Readiness 假阻断：当 15 个样本完整、B0 全程置位、同批样本的 operation B4 全程为 Measuring，且已有的 `INIT:CONT?` 回读精确为 1 时，B0 项降为非阻断 WARN。B0 发生变化、证据不完整、B4 不连续或 `INIT:CONT` 不是 1 仍 fail-closed；questionable B8/B9、reading overflow 及其他原有检查仍独立阻断。该规则不推断 FULL/LOW ACAL、Autozero 或根因，也不增加 ACAL 自动流程；ACAL 操作继续属于实验预备流程。

## v1.5.2 动态 inventory 边界
...
2182A 的 B0 汇总使用开始、末端和 13 个附加样本的主机单调时间。15 个样本完整、时间严格递增且 B0 全程清除时为 PASS；B0 全程置位时，仅在同批样本 B4 全程 Measuring 且 `INIT:CONT=1` 时为非阻断 WARN。B0 发生变化、B4 不连续或非连续启动仍为 BLOCKED；缺样本、查询失败、非法 condition word 或时间证据无效为 UNKNOWN 并阻断 Live。该短窗不是校准超时阈值，也不证明前面板 FULL/LOW ACAL 是否完成。
...
2182A 的 operation/measurement/questionable condition 使用 2182A 手册的独立 mask。operation B0 只在上述完整稳定 Measuring 窗口中降为 WARN；reading overflow、questionable B8 invalid calibration constant 和 questionable B9 ACAL condition 仍独立阻断；thermocouple reference condition 只警告。B9 只说明 ACAL 条件需要处理，不由程序推断具体原因。`SYST:POSETUP?` 接受手册规定的 `RST`、`PRES/PRESET` 和 `SAV0`。不会把 6221 的 B10 Idle 规则套到 `INIT:CONT=1` 的 2182A。
...
1. 校验发布 ZIP：
   ...
   ```powershell
   Get-FileHash "C:\LabAutomation\gpib6-2182a-diagnostic-core-v1.5.2.zip" -Algorithm SHA256
   ```

   作用：只读计算发布包哈希，不解压、不访问仪器。预期：与同名 `.sha256.txt` 和 v1.5.2 交接值完全一致；不覆盖旧版本目录。

2. 解压到全新目录：

   ```powershell
   Expand-Archive `
     "C:\LabAutomation\gpib6-2182a-diagnostic-core-v1.5.2.zip" `
     "C:\LabAutomation\gpib6-2182a-diagnostic-core-v1.5.2"
   ```
...
5. 运行离线测试：
   ...
   ```powershell
   Set-Location "C:\LabAutomation\gpib6-2182a-diagnostic-core-v1.5.2"
   & "C:\LabAutomation\.venv\Scripts\python.exe" -m unittest discover -s tests -t . -v
   ```

   作用：验证动态 inventory、单次身份查询、三种共享 profile、异步 owner、首尾一致性、2182A B0 时序分类与窄化 Readiness、Live 隔离、manifest/events/interventions、纯样本 CSV 和故障矩阵；不会实例化真实 VISA 会话。预期：`Ran 177 tests`、`OK (skipped=4)`，没有 failure/error。

6. 启动 GUI：

   ```powershell
   & "C:\LabAutomation\gpib6-2182a-diagnostic-core-v1.5.2\START_KEITHLEY_QUERY_ONLY_DIAGNOSTICS.ps1"
   ```
...
`monitor_runs/YYYYMMDD-HHMMSS-mmm-<mode>-inventory-v1.5.2/`
...
`monitor_runs/YYYYMMDD-HHMMSS-mmm-<mode>-<target>-diagnostic-v1.5.2/`
...
v1.5.2 能证明：一次明确 refresh 中 VISA 实际报告的资源、每资源单次身份结果、精确型号对应的批准只读诊断、关键字段在同一 session 首尾的可观察一致性、2182A B0 在本次短观察窗内的 clear/changed/set/incomplete 分类、稳定 B0 是否同时具有全程 B4 Measuring 和 `INIT:CONT=1` 证据、主机异步结果归属、记录器可用性，以及唯一 GPIB6 2182A Live 的解析、时序、人工干预和主机侧证据一致性。

v1.5.2 不能证明：B0 的原因、FULL/LOW ACAL 或 Autozero 类型与历史完成时间、整个顺序 snapshot 是原子状态、未被 `list_resources()` 返回的地址上是否存在仪器、6221 DC setpoint、2450 trigger-model state、2450 未确认的命令语言、外部校准有效期、样品接线、低温系统热平衡、接地/屏蔽质量、真实噪声谱性能，或量程/NPLC/filter 对具体 device 是否最优。程序不自动执行 ACAL，不根据人工标签自动判断或控制后续流程，不自动修正配置，不触发、不 INIT、不复位，也不执行多仪器协同。
