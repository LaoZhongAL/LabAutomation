...
class MonitorTests(unittest.TestCase):
    ...
    def test_simulated_configuration_matches_real_target(self):
        ...
        self.assertEqual(report["query_plan"]["executed_count"], 51)
        observation = report["operation_condition_observation"]
        self.assertEqual(observation["classification"], "clear_for_entire_window")
        self.assertEqual(observation["valid_sample_count"], 15)
        self.assertEqual(observation["b0_set_count"], 0)
        timestamps = [
            sample["session_elapsed_seconds"] for sample in observation["samples"]
        ]
    ...
    def test_operation_observation_blocks_when_b0_changes_during_window(self):
        ...
        observation_check = next(
            check
            for check in report["diagnostics"]["checks"]
            if check["check_id"] == "acquisition.operation_observation"
        )
        self.assertEqual(consistency["status"], "PASS")
        self.assertEqual(
            report["operation_condition_observation"]["classification"],
            "changed_during_window",
        )
        self.assertEqual(observation_check["status"], "BLOCKED")
        self.assertFalse(report["live_readiness"]["ready"])

    def test_stable_b0_with_measuring_and_continuous_initiation_warns(self):
        for operation_word in ("17", "273"):
            with self.subTest(operation_word=operation_word):
                class StableMeasuringSession:
                    def __enter__(self):
                        return self

                    def query(self, command):
                        if command == "STAT:OPER:COND?":
                ...
                with mock.patch.object(
                    monitor,
                    "session_factory",
                    return_value=StableMeasuringSession(),
                ):
                    report = monitor.collect_configuration("simulate")
                ...
                check = next(
                    ...
                    if item["check_id"] == "acquisition.operation_observation"
                )
                self.assertEqual(
                    observation["classification"], "set_for_entire_window"
                )
                self.assertEqual(observation["b0_set_count"], 15)
                self.assertEqual(check["status"], "WARN")
                self.assertFalse(check["blocks_live"])
                self.assertIn("Stable B0 alone", check["message"])
                self.assertTrue(report["diagnostics"]["diagnostics_acceptable"])
                self.assertTrue(report["live_readiness"]["ready"])

    def test_stable_b0_without_measuring_or_continuous_initiation_blocks(self):
        for operation_word, continuous_initiation in (("1", "1"), ("17", "0")):
            with self.subTest(
                operation_word=operation_word,
                continuous_initiation=continuous_initiation,
            ):
                class IncompatibleStableSession:
                    def __enter__(self):
                        return self
                ...
                with mock.patch.object(
                    monitor,
                    "session_factory",
                    return_value=IncompatibleStableSession(),
                ):
                    report = monitor.collect_configuration("simulate")
                ...
                self.assertTrue(check["blocks_live"])
                self.assertFalse(report["live_readiness"]["ready"])

    def test_stable_b0_warning_does_not_override_independent_danger_bits(self):
        for command, response, blocker_id in (
            (
                "STAT:QUES:COND?",
                "512",
                "calibration.condition.acal_questionable",
            ),
            ("STAT:MEAS:COND?", "1", "acquisition.reading_overflow"),
        ):
            with self.subTest(command=command):
                class IndependentDangerSession:
                    def __enter__(self):
                        return self

                    def query(self, current_command):
                        if current_command == "STAT:OPER:COND?":
                            return "273"
                        if current_command == command:
                            return response
                        return monitor.SIMULATED_VALUES[current_command]
                ...
                with mock.patch.object(
                    monitor,
                    "session_factory",
                    return_value=IndependentDangerSession(),
                ):
                    report = monitor.collect_configuration("simulate")

                checks = {
                    item["check_id"]: item for item in report["diagnostics"]["checks"]
                }
                self.assertEqual(
                    checks["acquisition.operation_observation"]["status"], "WARN"
                )
                self.assertFalse(
                    checks["acquisition.operation_observation"]["blocks_live"]
                )
                self.assertEqual(checks[blocker_id]["status"], "BLOCKED")
                self.assertFalse(report["live_readiness"]["ready"])

    def test_operation_observation_query_failure_is_insufficient_and_blocks(self):
            ...
            def query(self, command):
                if command == "STAT:OPER:COND?":
                    ...
                    if self.operation_count == 5:
                        raise TimeoutError("operation observation timeout")
                return monitor.SIMULATED_VALUES[command]
        ...
        with mock.patch.object(
            monitor,
            "session_factory",
            return_value=ProbeFailureSession(),
        ):
            report = monitor.collect_configuration("simulate")
        ...
        self.assertEqual(observation["classification"], "insufficient_evidence")
