...
PROFILE_ID = "keithley-2182a-read-only-v1.5.2"

OPERATION_CONDITION_COMMAND = "STAT:OPER:COND?"
...
OPERATION_OBSERVATION_NAMES = tuple(
    f"operation_observation.{index:02d}"
    for index in range(len(OPERATION_OBSERVATION_OFFSETS_SECONDS))
)
OPERATION_OBSERVATION_EXPECTED_NAMES = (
    "operation_condition",
    "snapshot_end.operation_condition",
    *OPERATION_OBSERVATION_NAMES,
)
_2182A_OPERATION_CONDITION_MASK = 0x0531
...
def _check_value(
    ...
    *,
    ...
    return CheckResult(
        ...
        expected,
        observed,
        f"{name} satisfies the required rule: {expected}."
        if passed
        else (
            f"{name} does not satisfy the required rule: {expected}; "
...
def summarize_2182a_operation_observation(
    transcript: Sequence[Mapping[str, object]],
) -> dict[str, object]:
    """Decode the fixed B0 observation window without inferring its source."""

    items_by_name: dict[str, list[Mapping[str, object]]] = {}
    for item in transcript:
        if isinstance(item, Mapping):
            items_by_name.setdefault(str(item.get("name")), []).append(item)
    ...
    for name in OPERATION_OBSERVATION_EXPECTED_NAMES:
        matches = items_by_name.get(name, [])
        item = matches[0] if len(matches) == 1 else None
        raw = item.get("response") if item is not None else None
        elapsed_raw = item.get("session_elapsed_seconds") if item is not None else None
        sample: dict[str, object] = {
            "name": name,
            "raw_word": raw,
            "session_elapsed_seconds": elapsed_raw,
            "valid": False,
            "b0_calibrating": None,
            "b4_measuring": None,
            "b10_idle": None,
        ...
        try:
            if item is None or not item.get("ok"):
                raise ValueError("missing or failed query")
            elapsed = _finite_float(elapsed_raw)
            word = _integer(raw)
            if elapsed < 0.0 or elapsed <= previous_elapsed:
                raise ValueError("timestamps are not strictly increasing")
            if not 0 <= word <= 65535 or word & ~_2182A_OPERATION_CONDITION_MASK:
                raise ValueError("condition word is outside the documented mask")
        except (TypeError, ValueError) as exc:
            complete = False
            ...
            sample.update(
                {
                    ...
                    "valid": True,
                    "b0_calibrating": b0,
                    "b4_measuring": bool(word & (1 << 4)),
                    "b10_idle": bool(word & (1 << 10)),
                }
            )
            b0_states.append(b0)
    ...
    if not complete or len(b0_states) != len(OPERATION_OBSERVATION_EXPECTED_NAMES):
        classification = "insufficient_evidence"
    elif all(not state for state in b0_states):
        classification = "clear_for_entire_window"
    elif all(b0_states):
        classification = "set_for_entire_window"
    else:
        classification = "changed_during_window"

    return {
        "protocol": {
            "command": OPERATION_CONDITION_COMMAND,
            "dedicated_duration_seconds": OPERATION_OBSERVATION_OFFSETS_SECONDS[-1],
            "dedicated_sample_count": len(OPERATION_OBSERVATION_NAMES),
            "total_expected_sample_count": len(OPERATION_OBSERVATION_EXPECTED_NAMES),
            "scheduled_offsets_seconds": list(OPERATION_OBSERVATION_OFFSETS_SECONDS),
        },
        "classification": classification,
        "valid_sample_count": len(b0_states),
        "b0_set_count": sum(b0_states),
        "b0_transition_count": sum(
            before != after for before, after in zip(b0_states, b0_states[1:])
        ),
        "observed_window_seconds": (
            round(elapsed_values[-1] - elapsed_values[0], 6)
...
def _2182a_operation_observation_check(
    observation: Mapping[str, object],
    values: Mapping[str, str],
) -> CheckResult:
    classification = str(observation.get("classification", "insufficient_evidence"))
    sample_count = observation.get("valid_sample_count")
    set_count = observation.get("b0_set_count")
    window = observation.get("observed_window_seconds")
    observed = {
        "classification": classification,
        "valid_sample_count": sample_count,
        "b0_set_count": set_count,
        "b0_transition_count": observation.get("b0_transition_count"),
        "observed_window_seconds": window,
    }
    samples = observation.get("samples", ())
    measuring_for_entire_window = (
        isinstance(samples, Sequence)
        and len(samples) == len(OPERATION_OBSERVATION_EXPECTED_NAMES)
        and all(
            isinstance(sample, Mapping)
            and sample.get("valid") is True
            and sample.get("b4_measuring") is True
            for sample in samples
        )
    )
    try:
        continuous_initiation = _integer(values.get("continuous_initiation")) == 1
    except (TypeError, ValueError):
        continuous_initiation = False
    blocks_live = True
    if classification == "clear_for_entire_window":
        status = CheckStatus.PASS
        message = (
            f"Operation B0 was clear at all {sample_count} sampled instants over "
            f"the {window} s observation window."
        )
    elif classification == "changed_during_window":
        status = CheckStatus.BLOCKED
        message = (
            f"Operation B0 was set at {set_count} of {sample_count} sampled instants "
            f"and changed during the {window} s observation window; its source is not determined."
        )
    elif classification == "set_for_entire_window":
        if measuring_for_entire_window and continuous_initiation:
            status = CheckStatus.WARN
            blocks_live = False
            message = (
                f"Operation B0 and Measuring B4 were both set at all {sample_count} "
                f"sampled instants over the {window} s observation window while "
                "continuous initiation was enabled. Stable B0 alone does not identify "
                "an unfinished ACAL, Autozero, or another specific source."
            )
        else:
            status = CheckStatus.BLOCKED
            message = (
                f"Operation B0 was set at all {sample_count} sampled instants over the "
                f"{window} s observation window without complete evidence of continuous "
                "Measuring B4; its source is not determined."
            )
    else:
        status = CheckStatus.UNKNOWN
        message = "Operation B0 observation is incomplete or contains invalid evidence."
    return CheckResult(
        "acquisition.operation_observation",
        ReadinessLayer.ACQUISITION,
        status,
        blocks_live,
        (
            "operation-condition B0 clear, or stable B0 with Measuring B4 at every "
            "sampled instant while continuous initiation is enabled"
        ),
        observed,
        message,
    )
...
def _2182a_key_status_checks(
    ...
    *,
    ...
    danger_bits = (
        ...
        (
            "calibration.condition.acal_questionable",
            ReadinessLayer.CONFIGURATION,
            "questionable_condition",
            9,
            CheckStatus.BLOCKED,
            True,
            (
                "The 2182A reports a questionable ACAL condition; "
                "the specific cause is not inferred."
            ),
        ),
        (
    ...
    if include_calibrating:
        danger_bits = (
            (
                ...
                CheckStatus.BLOCKED,
                True,
                "Operation-condition B0 is set in this diagnostic snapshot; its source is not determined.",
            ),
            *danger_bits,
        )
...
def evaluate_readiness(
    ...
    *,
    ...
    consistency_names: Sequence[str] = (),
    operation_observation: Mapping[str, object] | None = None,
) -> ReadinessReport:
    ...
    *,
    ...
    if family == "2182a":
        ...
        validators = (
            ...
            ("configuration.power_on_setup", "power_on_setup", "RST, PRES/PRESET, or SAV0", text, lambda v: v in {"RST", "PRES", "PRESET", "SAV0"}, True, CheckStatus.BLOCKED),
        ...
        checks.extend(
            _2182a_key_status_checks(
                values,
                include_calibrating=operation_observation is None,
            )
        )
        ...
        if operation_observation is not None:
            checks.append(
                _2182a_operation_observation_check(operation_observation, values)
            )
        checks.append(
            CheckResult(
