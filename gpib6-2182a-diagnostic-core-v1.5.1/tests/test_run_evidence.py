...
class RunEvidenceTests(unittest.TestCase):
    def test_release_identity_is_written_consistently(self):
        self.assertEqual(APP_RELEASE_TAG, "v1.5.1")
        self.assertEqual(APP_VERSION, "1.5.1-query-only-b0-observation")
        self.assertEqual(DIAGNOSTIC_SCHEMA_VERSION, 4)
        self.assertTrue(
            all(
                profile_id_for(target_key).endswith("-v1.5.1")
    ...
    def test_manifest_and_jsonl_lifecycle(self):
        ...
        self.assertEqual(manifest["statistics"]["error_count"], 0)
        self.assertEqual(manifest["artifacts"]["errors"], "errors.jsonl")
    ...
    def test_error_journal_captures_blocked_checks_and_runtime_faults(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            run_dir = Path(temp_dir) / "run"
            run_dir.mkdir()
            journal = RunJournal(
            ...
            journal.set_configuration_snapshot("configuration-snapshot.json")
            journal.set_diagnostics(
                observed_identity={"serial": "1340129"},
                readiness={
                    "overall": "BLOCKED",
                    "diagnostics_acceptable": False,
                    "can_start_live": False,
                    "live_authorized": True,
                    "blockers": ["The 2182A is calibrating."],
                    "warnings": [],
                    "checks": [
                        {
                            "check_id": "acquisition.calibrating",
                            "layer": "acquisition",
                            "status": "BLOCKED",
                            "blocks_live": True,
                            "expected": "operation_condition bit 0 clear",
                            "observed": True,
                            "message": "The 2182A is calibrating.",
                        }
                    ],
                },
            )
        ...
        self.assertEqual(
            errors,
            [event for event in events if event["severity"] == "ERROR"],
        )
        self.assertEqual(manifest["statistics"]["error_count"], 2)
        readiness_error = errors[0]
        self.assertEqual(readiness_error["event_type"], "readiness_evaluated")
        self.assertEqual(
            readiness_error["payload"]["configuration_snapshot"],
            "configuration-snapshot.json",
        )
        self.assertEqual(
            readiness_error["payload"]["target"]["resource"],
            "GPIB0::6::INSTR",
        )
        self.assertEqual(
            readiness_error["payload"]["failed_checks"][0]["observed"],
            True,
        )
        self.assertEqual(errors[1]["payload"]["message"], "TimeoutError: simulated timeout")
