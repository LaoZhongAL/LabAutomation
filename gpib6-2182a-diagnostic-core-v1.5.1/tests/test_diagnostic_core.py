...
class DiagnosticCoreTests(unittest.TestCase):
    ...
    def test_2182a_power_on_setup_accepts_documented_preset_forms_only(self):
        expected_statuses = {
            "RST": CheckStatus.PASS,
            "PRES": CheckStatus.PASS,
            "PRESET": CheckStatus.PASS,
            "SAV0": CheckStatus.PASS,
            "SAV1": CheckStatus.BLOCKED,
        }
        for raw, expected_status in expected_statuses.items():
            with self.subTest(raw=raw):
                values = dict(NOMINAL_VALUES, power_on_setup=raw)
                report = evaluate_readiness(
                    values,
                    nominal_transcript(values),
                    tuple(NOMINAL_VALUES),
                ...
                check = next(
                    ...
                    if item.check_id == "configuration.power_on_setup"
                )
                self.assertEqual(check.status, expected_status)
                self.assertNotIn("approved baseline", check.message)
                if expected_status == CheckStatus.BLOCKED:
                    self.assertIn("observed 'SAV1'", check.message)
