"""Thread-safe manifest and event journal for one query-only diagnostic run."""
...
from evidence_verifier import verify_run_directory, write_verification_report


APP_NAME = "gpib6-2182a-live-monitor"
APP_RELEASE_TAG = "v1.5.1"
APP_VERSION = "1.5.1-query-only-b0-observation"
...
_TSP_ATTRIBUTE_PRINT = re.compile(
    r"print\([A-Za-z_][A-Za-z0-9_]*(?:\.[A-Za-z_][A-Za-z0-9_]*)+\)"
)


def validate_query_allowlist(
    allowed_queries: Iterable[str],
    *,
    command_set: str,
) -> tuple[str, ...]:
    """Validate protocol syntax without weakening the exact allowlist boundary.

    TSP is executable code, so this validator deliberately accepts only a
    single ``print(dotted.attribute)`` expression plus the universal ``*IDN?``
    identity query.  Function calls, assignments, compound commands, and user
    expressions remain outside the policy even when wrapped in ``print()``.
    """

    items = list(allowed_queries)
    if not items:
        raise RecorderError("query allowlist must not be empty")
    protocol = str(command_set).strip().upper()
    if protocol not in {"SCPI", "TSP"}:
        raise RecorderError(f"unsupported command set: {command_set!r}")
    ...
    for command in items:
        ...
        if not structurally_safe:
            ...
            continue
        if protocol == "SCPI":
            safe = command.endswith("?")
        else:
            safe = command == "*IDN?" or _TSP_ATTRIBUTE_PRINT.fullmatch(command) is not None
        if not safe:
            unsafe.append(command)
    if unsafe:
        raise RecorderError(f"non-query or compound allowlist entries: {unsafe!r}")
...
class RunJournal:
    ...
    def __init__(
        ...
        *,
        ...
        queries = list(
            validate_query_allowlist(allowed_queries, command_set=command_set)
        )
        protocol = str(command_set).strip().upper()
        manifest_target = dict(target) if target is not None else target_as_dict()
        if not manifest_target:
            raise RecorderError("manifest target must not be empty")
        if not isinstance(profile_id, str) or not profile_id.strip():
            raise RecorderError("profile_id must not be empty")
        self.manifest_path = self.run_directory / "run-manifest.json"
        self.events_path = self.run_directory / "events.jsonl"
        self.errors_path = self.run_directory / "errors.jsonl"
        self.interventions_path = self.run_directory / "interventions.jsonl"
        if (
            self.manifest_path.exists()
            or self.events_path.exists()
            or self.errors_path.exists()
            or self.interventions_path.exists()
        ):
            raise RecorderError(
                "manifest, event journal, error journal, or intervention journal already exists"
            )
    ...
    def record_event(
        ...
        *,
        ...
        with self._lock:
            ...
            if severity == "ERROR":
                ...
                event_payload = {
                    ...
                    "configuration_snapshot": (
                        artifacts.get("configuration_snapshot")
            ...
            if severity == "ERROR":
                try:
                    with self.errors_path.open("a", encoding="utf-8", newline="\n") as handle:
                        handle.write(
                    ...
                    raise RecorderError(
                        f"error journal write failed: {type(exc).__name__}: {exc}"
                    ) from exc
                statistics = self.manifest["statistics"]
                if isinstance(statistics, dict):
                    statistics["error_count"] = int(
    ...
    def set_diagnostics(
        ...
        *,
        ...
        with self._lock:
            ...
            diagnostics_acceptable = readiness.get("diagnostics_acceptable") is True
            ...
            failed_checks = (
                [
                    ...
                    and check.get("status") in {"BLOCKED", "UNKNOWN"}
                ]
            ...
            self.record_event(
                "readiness_evaluated",
                severity="INFO" if diagnostics_acceptable else "ERROR",
                reason_code=(
                ...
                payload={
                    ...
                    "live_authorized": readiness.get("live_authorized"),
    ...
    def set_stream_quality(self, stream_id: str, filename: str) -> None:
        with self._lock:
            self._stream(stream_id)["quality"] = str(filename)
            self.record_event(
                "stream_quality_persisted",
                reason_code="STREAM_QUALITY_PERSISTED",
                payload={"path": str(filename)},
                stream_id=stream_id,
            )
            self._atomic_write_manifest()
    ...
    def finalize(self, reason: str) -> None:
        with self._lock:
            ...
            self._atomic_write_manifest()
            verification = verify_run_directory(self.run_directory)
            ...
            artifacts = self.manifest["artifacts"]
            if isinstance(artifacts, dict):
                artifacts["evidence_verification"] = verification_path.name
            if isinstance(final, dict):
                final["evidence_complete"] = bool(verification.get("passed"))
            readiness = self.manifest.get("readiness")
            health_axes = (
                readiness.get("health_axes")
                if isinstance(readiness, dict)
                else None
            )
            if isinstance(health_axes, dict):
                health_axes["evidence_complete"] = (
                    "PASS" if verification.get("passed") else "BLOCKED"
                )
            self._atomic_write_manifest()
            self._closed = True
