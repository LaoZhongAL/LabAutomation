...
from diagnostic_core import parse_idn, summarize_2182a_operation_observation
...
def verify_run_directory(run_directory: Path) -> dict[str, object]:
    ...
    if configuration_name is not None:
        try:
            ...
            if isinstance(configuration, dict):
                ...
                operation_observation = configuration.get(
                    "operation_condition_observation"
                )
                if isinstance(expected_identity, dict) and expected_identity.get("model") == "2182A":
                    expected_operation_observation = summarize_2182a_operation_observation(
                        transcript_items
                    )
                    record(
                        "configuration_snapshot.operation_observation",
                        operation_observation == expected_operation_observation,
                        "2182A operation observation must be derived exactly from transcript evidence",
                    )
                else:
                    record(
                        "configuration_snapshot.operation_observation",
                        operation_observation is None,
                        "non-2182A profiles must not contain an operation observation",
                    )
    ...
    try:
        error_events = _read_jsonl(_artifact_path(root, artifacts.get("errors")))
    except (OSError, UnicodeError, ValueError) as exc:
        error_events = []
        record("errors.readable", False, f"{type(exc).__name__}: {exc}")
    else:
        record("errors.readable", True, f"{len(error_events)} JSONL records")
        ...
        record(
            "errors.match_events",
            error_events == expected_error_events,
            "errors.jsonl must exactly mirror ERROR-severity events.jsonl records",
        )
    ...
    record(
        ...
        isinstance(statistics, dict)
        and statistics.get("error_count") == len(error_events),
        f"statistics.error_count must equal error journal total {len(error_events)}",
    )
