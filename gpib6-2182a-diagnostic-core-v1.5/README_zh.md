# GPIB0 多仪器只读诊断与 GPIB6 2182A Live v1.5

本目录包含 v1.5 动态 instrument inventory、按精确型号复用的只读诊断 profile，以及唯一获批的 GPIB6 Keithley 2182A 实时电压 GUI。v1.5 保留 v1.1 的记录语义：电压 CSV 只保存仪器样本，人工干预的类型、位置和开始/结束区间只保存到独立 `interventions.jsonl`。

v1.5 完成 P1：切换 inventory target 时清除不适用的故障场景；所有后台结果携带不可变操作归属，迟到事件在修改 UI 前被拒绝；2182A 增加官方批准的 autozero、FAZERO、LSYNC 与三个非消费 condition 查询；2450 只增加三个精确 condition 属性；三型号在同一 VISA session 末端复读各自关键字段并检查稳定性。6221 DC setpoint 仍不可观察，程序不猜测未批准查询。质量统计不包含自动阈值，不证明样品的物理性能。

## v1.5 动态 inventory 边界
...
一次 nominal 诊断的候选/执行计数按型号计算：2182A `38/38`（28 条基础查询加 10 条末端复读）；6221 `21/21`；2450 `41/43`，其中与当前 source function 不适用的两条 compliance 分支明确跳过。`38/38` 属于 2182A 共享 profile，不属于 GPIB6 地址；real 模式先读取 inventory 和身份，再按识别出的精确型号选择 profile。末端复读仍使用当前型号的相同 exact allowlist、同一个 VISA session、无 retry。稳定字段按布尔、有限数值或规范化文本比较；实时 condition word 不要求逐字相等，但开始端和结束端都分别检查合法域与型号专属危险状态。
...
2182A 的 operation/measurement/questionable condition 使用 2182A 手册的独立 mask：calibrating、reading overflow、invalid calibration constant 和 invalid ACAL 阻断；thermocouple reference condition 只警告。不会把 6221 的 B10 Idle 规则套到 `INIT:CONT=1` 的 2182A。
...
1. 校验发布 ZIP：
   ...
   ```powershell
   Get-FileHash "C:\LabAutomation\gpib6-2182a-diagnostic-core-v1.5.zip" -Algorithm SHA256
   ```

   作用：只读计算发布包哈希，不解压、不访问仪器。预期：与同名 `.sha256.txt` 和 v1.5 交接值完全一致；不覆盖旧版本目录。

2. 解压到全新目录：

   ```powershell
   Expand-Archive `
     "C:\LabAutomation\gpib6-2182a-diagnostic-core-v1.5.zip" `
     "C:\LabAutomation\gpib6-2182a-diagnostic-core-v1.5"
   ```
...
5. 运行离线测试：
   ...
   ```powershell
   Set-Location "C:\LabAutomation\gpib6-2182a-diagnostic-core-v1.5"
   & "C:\LabAutomation\.venv\Scripts\python.exe" -m unittest discover -s tests -t . -v
   ```

   作用：验证动态 inventory、单次身份查询、三种共享 profile、异步 owner、首尾一致性、Live 隔离、manifest/events/interventions、纯样本 CSV 和故障矩阵；不会实例化真实 VISA 会话。预期：`Ran 168 tests`、`OK (skipped=4)`，没有 failure/error。

6. 启动 GUI：

   ```powershell
   & "C:\LabAutomation\gpib6-2182a-diagnostic-core-v1.5\START_KEITHLEY_QUERY_ONLY_DIAGNOSTICS.ps1"
   ```
...
4. 选择 GPIB7 2182A 并运行诊断。预期复用 2182A profile并显示 `38/38`，但 Live/Single 仍禁用。
...
`monitor_runs/YYYYMMDD-HHMMSS-mmm-<mode>-inventory-v1.5/`
...
`monitor_runs/YYYYMMDD-HHMMSS-mmm-<mode>-<target>-diagnostic-v1.5/`
...
v1.5 能证明：一次明确 refresh 中 VISA 实际报告的资源、每资源单次身份结果、精确型号对应的批准只读诊断、关键字段在同一 session 首尾的可观察一致性、主机异步结果归属、记录器可用性，以及唯一 GPIB6 2182A Live 的解析、时序、人工干预和主机侧证据一致性。

v1.5 不能证明：整个顺序 snapshot 是原子状态、未被 `list_resources()` 返回的地址上是否存在仪器、6221 DC setpoint、2450 trigger-model state、2450 未确认的命令语言、外部校准有效期、样品接线、低温系统热平衡、接地/屏蔽质量、真实噪声谱性能，或量程/NPLC/filter 对具体 device 是否最优。程序不根据人工标签自动判断或控制后续流程，不自动修正配置，不触发、不 INIT、不复位，也不执行多仪器协同。
