...
class FaultInjectionTests(unittest.TestCase):
    def test_every_scenario_constructs_with_its_rule_allowlist(self):
        for scenario in FAULT_SCENARIO_NAMES:
            allowed = {rule.command for rule in FAULT_SCENARIOS[scenario].rules}
            SimulationContext(scenario, allowed, sleep=lambda _seconds: None)
    ...
    def test_configuration_close_failure_is_host_side_only(self):
        context = SimulationContext("configuration_close_failure", ALLOWED)

        self.assertTrue(context.should_fail_config_session_close())
